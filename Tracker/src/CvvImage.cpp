...
bool CvvImage::Create(int w, int h, int bpp, int origin) {
    ...
    if ((bpp != 8 && bpp != 24 && bpp != 32) || (unsigned)w >= max_img_size ||
        (unsigned)h >= max_img_size ||
    ...
    if (!m_img || Bpp() != bpp || m_img->width != w || m_img->height != h) {
        if (m_img && m_img->nSize == sizeof(IplImage)) Destroy();

        /* prepare IPL header */
        m_img = cvCreateImage(cvSize(w, h), IPL_DEPTH_8U, bpp / 8);
    }

    if (m_img) m_img->origin = origin == 0 ? IPL_ORIGIN_TL : IPL_ORIGIN_BL;
...
bool CvvImage::LoadRect(const char* filename, int desired_color, CvRect r) {
    ...
    if (r.x > img->width || r.y > img->height || r.x + r.width < 0 ||
        r.y + r.height < 0) {
        cvReleaseImage(&img);
        return false;
    }
...
void CvvImage::Show(HDC dc, int x, int y, int w, int h, int from_x,
    ...
    if (m_img && m_img->depth == IPL_DEPTH_8U) {
        ...
        SetDIBitsToDevice(dc, x, y, sw, sh, from_x, from_y, from_y, sh,
                          m_img->imageData + from_y * m_img->widthStep, bmi,
                          DIB_RGB_COLORS);
    }
}
...
void CvvImage::DrawToHDC(HDC hDCDst, RECT* pDstRect) {
    if (pDstRect && m_img && m_img->depth == IPL_DEPTH_8U && m_img->imageData) {
        ...
        ::StretchDIBits(hDCDst, dst.x, dst.y, dst.width, dst.height, roi.x,
                        roi.y, roi.width, roi.height, m_img->imageData, bmi,
                        DIB_RGB_COLORS, SRCCOPY);
    }
}

void CvvImage::Fill(int color) {
    cvSet(m_img, cvScalar(color & 255, (color >> 8) & 255, (color >> 16) & 255,
                          (color >> 24) & 255));
}
#endif
